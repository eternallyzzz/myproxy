...
func Inbound(ctx context.Context, s *model.Service, conn *quic.Conn) {
	...
	zlog.Info("listening UDP on " + udpAddr.String())
	...
	zlog.Info("listening TCP on " + tl.Addr().String())

	for {
		client, err := tl.Accept()
		if err != nil {
			zlog.Error("Failed to accept client connection:", zap.Error(err))
			return
		}
...
func listenUDP(ctx context.Context, l *net.UDPConn, quicConn *quic.Conn) {
	...
	for {
		...
		zlog.Debug("client connection from " + addr.String())
		...
		if data[0] == 0 && data[1] == 0 {
			...
			if config.IPDB != nil {
				country, err := config.IPDB.Country(ip)
				if err != nil {
					zlog.Error(err.Error())
					continue
				}
			...
			if work.DstConn == nil {
				zlog.Debug("request udp to " + dstAddr.String() + " by " + quicConn.String())
...
func tcp(ctx context.Context, conn net.Conn, localAddr *net.UDPAddr, quicConn *quic.Conn) {
	...
	if supportAuth {
		...
		zlog.Debug(request.Username + " " + request.Password)
		// 回复验证通过
	...
	if request.Command == 1 {
		...
		if err != nil {
			zlog.Error("Failed to write SOCKS5 UDP ASSOCIATE response:", zap.Error(err))
			return
		}
		conn.Close()
	}
...
func outboundSocks(ctx context.Context, req socks5.Request, conn net.Conn, quicConn *quic.Conn) {
	zlog.Debug("request tcp to " + req.Destination.String() + " by " + quicConn.String())
	...
	if err != nil {
		zlog.Error("Failed to write SOCKS5 request response:", zap.Error(err))
		return
	}
...
func directSocks(req socks5.Request, conn net.Conn) {
	...
	zlog.Debug("request tcp to " + req.Destination.String() + " direct")
...
func (w *Work) read() {
	...
	for {
		...
		zlog.Debug("back response with " + strconv.Itoa(len(p.Content)) + "bytes")
